...
function buildLanguage(languageClass, legacySpeakers, copy) {
  ...
  return section(copy.language, `
    ...
    <div class="lesson-brief"><div><small>${copy.objective}</small><h3>${text(brief.objective)}</h3>${brief.executiveBrief ? `<p>${text(brief.executiveBrief)}</p>` : ''}</div><div><small>${copy.learners}</small><p>${text(array(brief.learnerSpeakers).join(', '))}</p></div>${evidence(brief.evidence, copy)}</div>
