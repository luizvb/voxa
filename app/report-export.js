...
function buildMeeting(meeting, copy) {
  ...
  return section(copy.meeting, `
    <div class="manager"><small>${copy.managerBrief}</small><h3>${text(brief.bottomLine || brief.outcome)}</h3>${brief.bottomLine ? `<p>${text(brief.outcome)}</p>` : ''}<div class="triple"><div><h4>${copy.changed}</h4>${list(brief.whatChanged)}</div><div><h4>${copy.decisionNeeded}</h4>${list(brief.needsDecision)}</div><div><h4>${copy.escalation}</h4>${list(brief.needsEscalation)}</div><div><h4>${copy.managementAttention}</h4>${list(brief.managementAttention)}</div></div></div>
    <div class="columns"><div><h3>${copy.decisions}</h3>${itemCards(meeting.decisions, 'decision', (item) => `<p>${text(item.impact)}</p>${item.rationale ? `<p>${text(item.rationale)}</p>` : ''}${item.tradeoffs ? `<p class="note">${text(item.tradeoffs)}</p>` : ''}${item.owner ? `<p class="note">${copy.owner}: ${text(item.owner)}</p>` : ''}`, copy, 'No confirmed decisions identified')}</div><div><h3>${copy.actions}</h3>${actions.map((item) => `<article><h4>${text(item.task)} ${item.priority ? `<span class="tag">${text(item.priority)}</span>` : ''}</h4><p><b>${copy.owner}:</b> ${text(item.owner, copy.unassigned)}</p><p><b>${copy.due}:</b> ${text(item.dueDate, copy.noDate)}</p>${item.expectedOutcome ? `<p class="note">${copy.expectedOutcome}: ${text(item.expectedOutcome)}</p>` : ''}${evidence(typeof item.evidence === 'string' ? [item.evidence] : item.evidence, copy)}</article>`).join('') || '<p class="muted">No explicit action items identified</p>'}</div></div>
...
function buildAnalysisReportHtml({ analysis, recording, locale = 'en-US' }) {
  ...
  const limitations = [
    ...array(safe.evidenceQuality?.limitations),
    ...array(safe.evidenceQuality?.missingInformation)
  ];
  return `<!doctype html><html><head><meta charset="utf-8"><title>${text(recording?.name, 'Voxa report')}</title><style>
    ...
    * { box-sizing: border-box; }
    ...
    <section class="cover"><div class="brand">Voxa<i>.</i></div><div class="hero"><span class="eyebrow">${copy.report}</span><h1>${text(summary.title || recording?.name, 'Conversation report')}</h1><p class="lead">${text(summary.executiveBrief?.statement || summary.overview)}</p></div>${summary.bottomLine?.statement ? `<div class="purpose"><small>${copy.bottomLine}</small><p>${text(summary.bottomLine.statement)}</p>${evidence(summary.bottomLine.evidence, copy)}</div>` : ''}${summary.purpose?.statement ? `<div class="purpose"><small>${copy.purpose}</small><p>${text(summary.purpose.statement)}</p></div>` : ''}<div><div class="meta"><div><small>${copy.recorded}</small><p>${escapeHtml(recordingDate)}</p></div><div><small>${copy.generated}</small><p>${escapeHtml(generatedAt)}</p></div><div><small>${copy.modes}</small><p>${text(array(safe.analysisModes).join(', '))}</p></div><div><small>${copy.quality}</small><p>${text(safe.evidenceQuality?.level)}</p></div></div>${array(summary.criticalFindings).length ? `<h3>${copy.criticalFindings}</h3>${itemCards(summary.criticalFindings, 'finding', (item) => `<p>${text(item.significance)}</p><p class="note">${copy.businessImpact}: ${text(item.businessImpact)}</p>`, copy, '')}` : ''}${array(summary.recommendedActions).length ? `<h3>${copy.recommendedActions}</h3>${itemCards(summary.recommendedActions, 'action', (item) => `<p>${text(item.rationale)}</p><p class="note">${copy.expectedOutcome}: ${text(item.expectedOutcome)}</p>`, copy, '')}` : ''}${array(summary.unansweredQuestions).length ? `<h3>${copy.executiveQuestions}</h3>${itemCards(summary.unansweredQuestions, 'question', (item) => `<p>${text(item.whyItMatters)}</p>`, copy, '')}` : ''}${array(summary.keyPoints).length ? `<div class="key-points"><h3>${copy.keyPoints}</h3><ol>${array(summary.keyPoints).map((item) => `<li><span>${text(item.statement || item)}</span></li>`).join('')}</ol></div>` : ''}${limitations.length ? `<h3>${copy.limitations}</h3>${list(limitations)}` : ''}<footer><b>Voxa:</b> ${copy.verify}</footer></div></section>
